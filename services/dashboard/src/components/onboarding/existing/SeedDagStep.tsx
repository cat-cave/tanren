    ...
    <>
      <div class="step-heading">
        <div>
          ...
          <div class="title">seed the <em>dag from reality</em></div>
          <div class="sub">the agent's gap analysis becomes seed specs · your open github issues become candidate specs · forge de-dupes by title.</div>
        </div>
        <div class="right"><span class="pill ok"><span class="d"></span>seeded from issues + gaps</span></div>
      </div>

      <div class="ex-legend">
        <span class="key"><span class="swatch"></span> from github issue</span>
        <span class="key"><span class="swatch gap"></span> from agent gap</span>
      </div>
...
<input type="hidden" name="repoUrl" value={props.repoUrl} />
          ...
          <div class="col-card live" style="flex-direction:row;align-items:center;gap:12px">
            <div style="font-family:var(--font-ui);font-size:12px;color:var(--fg-1);line-height:1.4;flex:1">
              seed creates pending specs from {props.report.gaps.length} recon gaps + your open github issues. nothing runs yet — these queue behind the integration pr.
            </div>
            <button type="submit" class="btn primary">seed the dag ↗</button>
          </div>
        </form>
      ) : (
        <>
          <div class="alert ok">
            Seeded <b>{props.seeded.seeded.length}</b> specs · {props.seeded.fromIssues} from issues · {props.seeded.fromGaps} from gaps · {props.seeded.duplicatesDropped} dupes dropped.
          </div>
          <div class="col-card" style="gap:6px">
            <div class="h"><span>seed specs</span></div>
...
<input type="hidden" name="repoUrl" value={props.repoUrl} />
            ...
            <div class="foot">
              ...
              <div class="grow"></div>
              <button type="submit" class="btn primary">next · governance ↗</button>
            </div>
          </form>
        </>
      )}
