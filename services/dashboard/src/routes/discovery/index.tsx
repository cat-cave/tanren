...
export function mountDiscoveryScreens(app: Hono, deps: ShellDeps): void {
  ...
  app.post("/projects/:projectId/discovery", async (c) => {
    ...
    const variant = resolveVariant(formField(form, "variant"));
    const seed = SEED_INSIGHTS[variant];
    const insight: DiscoveryInsight = {
      variant,
      source: formField(form, "source", seed.source) || seed.source,
      sourceLabel: formField(form, "sourceLabel", seed.sourceLabel) || seed.sourceLabel,
      who: formField(form, "who", seed.who) || seed.who,
      when: formField(form, "when", seed.when) || seed.when,
  ...
  app.post("/projects/:projectId/discovery/accept", async (c) => {
    ...
    const form = await c.req.parseBody();
    const variant = resolveVariant(formField(form, "variant"));
    const insight = safeJson<DiscoveryInsight>(formField(form, "insight"));
    const proposals = safeJson<ProposedSpec[]>(formField(form, "proposals")) ?? [];
    const placementKind = formField(form, "placementKind", "slot_after") as PlacementKind;
    const placementLabel = formField(form, "placementLabel").trim();
