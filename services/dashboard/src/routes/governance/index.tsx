 ...
 * POST /settings/governance proxies only auditPosture + revision to the
 * canonical org-admin PUT, then redirects so success and failures stay visible.
 */
...
export function mountGovernanceScreen(app: Hono, deps: ShellDeps): void {
  app.get("/settings/governance", async (c) => {
    ...
    // activeNavId "config" keeps settings chrome lit without a leased nav row.
    const ctx = await loadShellContext(c, deps, { activeNavId: "config", projectId: requestedId });
  ...
  app.post("/settings/governance", async (c) => {
    ...
    const ctx = await loadShellContext(c, deps, { activeNavId: "config", projectId });
    ...
    const revision = formField(form, "revision").trim();
    ...
    if (!posture.success || revision === "") return c.redirect(redirect(projectId, "err=invalid"));

    const result = await (await writeClient(c, deps)).putAuditPosture(ctx.org.id, projectId, posture.data, revision);
