/**
 * Canonical audit-posture client. Dashboard consumer only: maps 1:1 to the
 * existing org/project governance GET/PUT. No local store, generic project
 * PATCH, or compatibility route.
 */
...
export class GovernanceClient extends OrchestratorHttpClient {
  ...
  async putAuditPosture(
    ...
    const result = await this.sendJson("PUT", governancePath(orgId, projectId), {
      revision,
