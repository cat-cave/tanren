// @generated by scripts/gen-dashboard-types.mjs — DO NOT EDIT BY HAND.
// Client response types generated from contracts/json/http/** (the orchestrator's Zod export).
// Regenerate with `corepack pnpm run codegen:dashboard-types`;
// `check:dashboard-types-drift` fails when this file diverges from the schemas.
// See docs/architecture/future-refactor-and-scale.md §2.

/** Generated from `tanren.http.ProjectFeedItem` (contracts/json/http/ProjectFeedItem.json). */
export interface ProjectFeedItem {
  eventType: string;
  id: string;
  payload: unknown;
  projectId: string | null;
  redactedPaths: string[];
  runId: string;
  specId: string | null;
  taskId: string | null;
  ts: string;
}

/** Generated from `tanren.http.RunCostRecord` (contracts/json/http/RunCostRecord.json). */
export interface RunCostRecord {
  billingMode: "per_token" | "subscription" | "self_hosted" | "unattributed";
  cacheCreationTokens: number;
  cachedInputTokens: number;
  cli: string;
  costBasis: "ccusage" | "provider_response" | "credits" | "unknown" | "unattributed";
  costUsd: string | null;
  id: string;
  inputTokens: number;
  model: string;
  outputTokens: number;
  projectId: string;
  provider: string;
  reasoningOutputTokens: number;
  recordedAt: string;
  runId: string;
  taskId: string;
  totalTokens: number;
}

/** Generated from `tanren.http.RunDetail` (contracts/json/http/RunDetail.json). */
export interface RunDetail {
  costs: Array<{
    billingMode: "per_token" | "subscription" | "self_hosted" | "unattributed";
    cacheCreationTokens: number;
    cachedInputTokens: number;
    cli: string;
    costBasis: "ccusage" | "provider_response" | "credits" | "unknown" | "unattributed";
    costUsd: string | null;
    id: string;
    inputTokens: number;
    model: string;
    outputTokens: number;
    projectId: string;
    provider: string;
    reasoningOutputTokens: number;
    recordedAt: string;
    runId: string;
    taskId: string;
    totalTokens: number;
  }>;
  forgeThread: {
    recentTurns: unknown[];
    threadId: string;
  } | null;
  insights: unknown[];
  recentEvents: Array<{
    eventType: string;
    id: string;
    payload: unknown;
    projectId: string | null;
    redactedPaths: string[];
    runId: string | null;
    specId: string | null;
    taskId: string | null;
    ts: string;
  }>;
  run: {
    branch: string;
    endedAt: string | null;
    outcome:
      | "ok"
      | "halted"
      | "escape_hatch_hit"
      | "retry_budget_exhausted"
      | "convergence_stalled"
      | "window_exhausted"
      | "window_paused"
      | "awaiting_review"
      | "cancelled"
      | "failed"
      | null;
    prUrl: string | null;
    projectId: string;
    runId: string;
    specId: string;
    startedAt: string;
    status: "queued" | "running" | "paused" | "halted" | "completed" | "failed" | "cancelled";
    trigger: string;
  };
  spec: {
    behaviorIds: string[];
    description: string;
    milestoneId: string | null;
    specId: string;
    title: string;
  };
  tasks: Array<{
    attempt: number;
    cli: string;
    endedAt: string | null;
    failureKind: string | null;
    kind:
      | "plan"
      | "write"
      | "check"
      | "audit"
      | "ci"
      | "review"
      | "merge"
      | "demo"
      | "forge"
      | "triage"
      | "convergence"
      | "designOracle";
    model: string | null;
    outcome:
      | "passed"
      | "ok"
      | "pending"
      | "failed"
      | "rejected_by_checker"
      | "rejected_by_auditor"
      | "timed_out"
      | "crashed"
      | "window_exhausted"
      | "cancelled"
      | null;
    parentTaskId: string | null;
    runId: string;
    startedAt: string | null;
    status: "queued" | "claimed" | "running" | "done" | "failed" | "cancelled";
    taskId: string;
    title: string;
  }>;
}

/** Generated from `tanren.http.RunEventRow` (contracts/json/http/RunEventRow.json). */
export interface RunEventRow {
  eventType: string;
  id: string;
  payload: unknown;
  projectId: string | null;
  redactedPaths: string[];
  runId: string | null;
  specId: string | null;
  taskId: string | null;
  ts: string;
}

/** Generated from `tanren.http.RunListItem` (contracts/json/http/RunListItem.json). */
export interface RunListItem {
  branch: string;
  costTotalUsd: string;
  endedAt: string | null;
  lastEventAt: string | null;
  needsReview: boolean;
  outcome:
    | "ok"
    | "halted"
    | "escape_hatch_hit"
    | "retry_budget_exhausted"
    | "convergence_stalled"
    | "window_exhausted"
    | "window_paused"
    | "awaiting_review"
    | "cancelled"
    | "failed"
    | null;
  prUrl: string | null;
  projectId: string;
  runId: string;
  specId: string;
  specTitle: string;
  startedAt: string;
  status: "queued" | "running" | "paused" | "halted" | "completed" | "failed" | "cancelled";
  trigger: string;
}

/** Generated from `tanren.http.RunSpecSummary` (contracts/json/http/RunSpecSummary.json). */
export interface RunSpecSummary {
  behaviorIds: string[];
  description: string;
  milestoneId: string | null;
  specId: string;
  title: string;
}

/** Generated from `tanren.http.RunSummary` (contracts/json/http/RunSummary.json). */
export interface RunSummary {
  branch: string;
  endedAt: string | null;
  outcome:
    | "ok"
    | "halted"
    | "escape_hatch_hit"
    | "retry_budget_exhausted"
    | "convergence_stalled"
    | "window_exhausted"
    | "window_paused"
    | "awaiting_review"
    | "cancelled"
    | "failed"
    | null;
  prUrl: string | null;
  projectId: string;
  runId: string;
  specId: string;
  startedAt: string;
  status: "queued" | "running" | "paused" | "halted" | "completed" | "failed" | "cancelled";
  trigger: string;
}

/** Generated from `tanren.http.SseCostsFrame` (contracts/json/http/SseCostsFrame.json). */
export interface SseCostsFrame {
  costCursor: string;
  costs: Array<{
    billingMode: "per_token" | "subscription" | "self_hosted" | "unattributed";
    cacheCreationTokens: number;
    cachedInputTokens: number;
    cli: string;
    costBasis: "ccusage" | "provider_response" | "credits" | "unknown" | "unattributed";
    costUsd: string | null;
    id: string;
    inputTokens: number;
    model: string;
    outputTokens: number;
    projectId: string;
    provider: string;
    reasoningOutputTokens: number;
    recordedAt: string;
    runId: string;
    taskId: string;
    totalTokens: number;
  }>;
  projectId: string;
  runId: string;
}

/** Generated from `tanren.http.SseDrainedFrame` (contracts/json/http/SseDrainedFrame.json). */
export interface SseDrainedFrame {
  costCursor: string;
  eventCursor: string;
  outcome:
    | "ok"
    | "halted"
    | "escape_hatch_hit"
    | "retry_budget_exhausted"
    | "convergence_stalled"
    | "window_exhausted"
    | "window_paused"
    | "awaiting_review"
    | "cancelled"
    | "failed"
    | null;
  projectId: string;
  runId: string;
  status: "queued" | "running" | "paused" | "halted" | "completed" | "failed" | "cancelled";
  taskWatermark: string;
}

/** Generated from `tanren.http.SseEventsFrame` (contracts/json/http/SseEventsFrame.json). */
export interface SseEventsFrame {
  eventCursor: string;
  events: Array<{
    eventType: string;
    id: string;
    payload: unknown;
    projectId: string | null;
    redactedPaths: string[];
    runId: string | null;
    specId: string | null;
    taskId: string | null;
    ts: string;
  }>;
  projectId: string;
  runId: string;
}

/** Generated from `tanren.http.SseHeartbeatFrame` (contracts/json/http/SseHeartbeatFrame.json). */
export interface SseHeartbeatFrame {
  projectId: string;
  runId: string;
  ts: string;
}

/** Generated from `tanren.http.SseSnapshotFrame` (contracts/json/http/SseSnapshotFrame.json). */
export interface SseSnapshotFrame {
  costCursor: string;
  costs: Array<{
    billingMode: "per_token" | "subscription" | "self_hosted" | "unattributed";
    cacheCreationTokens: number;
    cachedInputTokens: number;
    cli: string;
    costBasis: "ccusage" | "provider_response" | "credits" | "unknown" | "unattributed";
    costUsd: string | null;
    id: string;
    inputTokens: number;
    model: string;
    outputTokens: number;
    projectId: string;
    provider: string;
    reasoningOutputTokens: number;
    recordedAt: string;
    runId: string;
    taskId: string;
    totalTokens: number;
  }>;
  eventCursor: string;
  projectId: string;
  recentEvents: Array<{
    eventType: string;
    id: string;
    payload: unknown;
    projectId: string | null;
    redactedPaths: string[];
    runId: string | null;
    specId: string | null;
    taskId: string | null;
    ts: string;
  }>;
  run: {
    branch: string;
    endedAt: string | null;
    outcome:
      | "ok"
      | "halted"
      | "escape_hatch_hit"
      | "retry_budget_exhausted"
      | "convergence_stalled"
      | "window_exhausted"
      | "window_paused"
      | "awaiting_review"
      | "cancelled"
      | "failed"
      | null;
    prUrl: string | null;
    projectId: string;
    runId: string;
    specId: string;
    startedAt: string;
    status: "queued" | "running" | "paused" | "halted" | "completed" | "failed" | "cancelled";
    trigger: string;
  };
  runId: string;
  taskWatermark: string;
  tasks: Array<{
    attempt: number;
    cli: string;
    endedAt: string | null;
    failureKind: string | null;
    kind:
      | "plan"
      | "write"
      | "check"
      | "audit"
      | "ci"
      | "review"
      | "merge"
      | "demo"
      | "forge"
      | "triage"
      | "convergence"
      | "designOracle";
    model: string | null;
    outcome:
      | "passed"
      | "ok"
      | "pending"
      | "failed"
      | "rejected_by_checker"
      | "rejected_by_auditor"
      | "timed_out"
      | "crashed"
      | "window_exhausted"
      | "cancelled"
      | null;
    parentTaskId: string | null;
    runId: string;
    startedAt: string | null;
    status: "queued" | "claimed" | "running" | "done" | "failed" | "cancelled";
    taskId: string;
    title: string;
  }>;
}

/** Generated from `tanren.http.SseStatusFrame` (contracts/json/http/SseStatusFrame.json). */
export interface SseStatusFrame {
  outcome:
    | "ok"
    | "halted"
    | "escape_hatch_hit"
    | "retry_budget_exhausted"
    | "convergence_stalled"
    | "window_exhausted"
    | "window_paused"
    | "awaiting_review"
    | "cancelled"
    | "failed"
    | null;
  projectId: string;
  runId: string;
  status: "queued" | "running" | "paused" | "halted" | "completed" | "failed" | "cancelled";
}

/** Generated from `tanren.http.SseTaskFrame` (contracts/json/http/SseTaskFrame.json). */
export interface SseTaskFrame {
  projectId: string;
  runId: string;
  task: {
    attempt: number;
    cli: string;
    endedAt: string | null;
    failureKind: string | null;
    kind:
      | "plan"
      | "write"
      | "check"
      | "audit"
      | "ci"
      | "review"
      | "merge"
      | "demo"
      | "forge"
      | "triage"
      | "convergence"
      | "designOracle";
    model: string | null;
    outcome:
      | "passed"
      | "ok"
      | "pending"
      | "failed"
      | "rejected_by_checker"
      | "rejected_by_auditor"
      | "timed_out"
      | "crashed"
      | "window_exhausted"
      | "cancelled"
      | null;
    parentTaskId: string | null;
    runId: string;
    startedAt: string | null;
    status: "queued" | "claimed" | "running" | "done" | "failed" | "cancelled";
    taskId: string;
    title: string;
  };
  taskWatermark: string;
}

/** Generated from `tanren.http.TaskTimelineEntry` (contracts/json/http/TaskTimelineEntry.json). */
export interface TaskTimelineEntry {
  attempt: number;
  cli: string;
  endedAt: string | null;
  failureKind: string | null;
  kind:
    | "plan"
    | "write"
    | "check"
    | "audit"
    | "ci"
    | "review"
    | "merge"
    | "demo"
    | "forge"
    | "triage"
    | "convergence"
    | "designOracle";
  model: string | null;
  outcome:
    | "passed"
    | "ok"
    | "pending"
    | "failed"
    | "rejected_by_checker"
    | "rejected_by_auditor"
    | "timed_out"
    | "crashed"
    | "window_exhausted"
    | "cancelled"
    | null;
  parentTaskId: string | null;
  runId: string;
  startedAt: string | null;
  status: "queued" | "claimed" | "running" | "done" | "failed" | "cancelled";
  taskId: string;
  title: string;
}
