...
import { ForgePublicationPanel } from "../src/components/runDetail/ReviewBody.js";
...
describe("reviewMergeStateFromEvents — P3-0008 review/merge phase", () => {
  it("renders every gv-2 publication state as an explicit data-state (published is receipt-backed)", async () => {
    ...
    for (const status of states) {
      ...
      expect(await (await app.request("/")).text()).toContain(`data-state="${status}"`);
    }
    const app = new Hono();
    app.get("/", (c) =>
      c.html(
        ForgePublicationPanel({
          phase: "approved",
          status: "published",
          publication: {
            complete: true,
    ...
    expect(html).toContain("pullrequestreview-7");
  });
  ...
  it("gv-2 former-bug: terminal review with ZERO forge fields is unpublished (undefined), not danger-partial", () => {
    ...
    expect(state.publicationStatus).toBe("non_simulated");
  });
  ...
  it("gv-2 UI states: matching intent + complete exact-head receipt is published", () => {
    const headSha = "a".repeat(40);
    const state = reviewMergeStateFromEvents([
      ev("review.simulated_intent", { headSha, state: "approved", reviewerLogin: "reviewer-bot" }),
      ev("review.approved", {
        reviewer: "reviewer-bot",
        forgeReviewId: "7",
        forgeReviewState: "approved",
  ...
  it("gv-2 UI states: receipt on a different intent/gate head is stale", () => {
    const headA = "a".repeat(40);
    const headB = "b".repeat(40);
    const state = reviewMergeStateFromEvents([
      ev("review.simulated_intent", { headSha: headA, state: "approved", reviewerLogin: "reviewer-bot" }),
      ev("review.approved", {
        reviewer: "reviewer-bot",
        forgeReviewId: "7",
        forgeReviewState: "approved",
      ...
      ev("gate.verdict", { when: "pre_merge", headSha: headB }),
    ]);
  ...
  it("gv-2 UI states: human/auto terminal without simulated receipt is non_simulated", () => {
    expect(reviewMergeStateFromEvents([ev("review.approved", { reviewer: "human" })]).publicationStatus).toBe(
      "non_simulated",
    );
    expect(reviewMergeStateFromEvents([ev("review.auto_approved")]).publicationStatus).toBe("non_simulated");
  });
