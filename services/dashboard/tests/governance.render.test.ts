    ...
    if (/\/orgs\/[^/]+\/projects\/[^/]+\/governance$/u.test(url)) {
      if (method === "PUT") {
        ...
        if (governancePutStatus >= 200 && governancePutStatus < 300) {
          ...
          const revision = String(Number((governancePayload as { revision: string }).revision) + 1);
          governancePayload = { ...(governancePayload as object), auditPosture, revision };
...
describe("audit-posture governance screen (/settings/governance)", () => {
  it("mounts via config thin-wire and renders the canonical current posture", async () => {
    ...
    // Nav row is leased (screens.ts/routes.ts); URL surface is registered once.
    expect(app.routes.filter((route) => route.method === "GET" && route.path === "/settings/governance")).toHaveLength(
      1,
    );
  });

  it("admin save sends only auditPosture + revision to the canonical PUT with outbound CSRF", async () => {
    const app = await build();
    const response = await postForm(
      app,
      "projectId=project_easy&revision=1&blockReviewAt=P3&p2p3Handling=route-to-dag&autonomousRemediation=true",
    );
  ...
  it("rejects invalid form values before any orchestrator write and leaves the error visible", async () => {
    ...
    const response = await postForm(
      app,
      "projectId=project_easy&revision=1&blockReviewAt=P9&p2p3Handling=fix-if-idle&autonomousRemediation=true",
    );
    expect(response.status).toBe(302);
  ...
  it("keeps an orchestrator server failure and unchanged current posture visible", async () => {
    ...
    const app = await build();
    const response = await postForm(app, "projectId=project_easy&revision=1&blockReviewAt=P2&p2p3Handling=fix-if-idle");
  ...
  it("renders malformed successful PUT confirmation as an unknown, actionable save outcome", async () => {
    ...
    // Malformed PUT response is not accepted as success; GET still shows prior posture.
    expect(html).toContain('data-current-block-review-at="P3"');
  });
  ...
  it("refuses an unknown project id before any governance write", async () => {
    ...
    const response = await postForm(
      app,
      "projectId=project_other&revision=1&blockReviewAt=P3&p2p3Handling=route-to-dag&autonomousRemediation=true",
    );
