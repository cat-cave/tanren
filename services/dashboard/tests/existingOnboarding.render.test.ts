...
const PROJECT = { projectId: "project_linked", name: "tanren-fixture-easy", repoUrl: "https://github.com/cat-cave/tanren-fixture-easy", defaultBranch: "main", runnerImage: null, allocator: "local_docker" };

const RECON_RESULT = {
  repoUrl: "https://github.com/cat-cave/tanren-fixture-easy",
  filesIndexed: 84,
  report: {
    identity: { slug: "tanren-fixture-easy", purpose: "smoke fixture for the tanren loop", inferredFrom: "README.md" },
...
const CONFIG_INJECTION_RESULT = {
  pullRequest: { number: 48, url: "https://github.com/cat-cave/tanren-fixture-easy/pull/48", branch: "tanren/integrate", filesCommitted: [".tanren/PROJECT.md", ".mergify.yml"] },
  files: [{ path: ".tanren/PROJECT.md", addedLines: 30 }, { path: ".mergify.yml", addedLines: 14 }],
  noRunsUntilMerged: true
};

const SEED_RESULT = {
  seeded: [
    { specId: "spec_1", title: "writer hangs on long writes", source: "github_issue", origin: "gh#142" },
    { specId: "spec_2", title: "default to industrial?", source: "agent_gap", origin: "design-dna" }
  ],
  duplicatesDropped: 1,
  fromIssues: 1,
  fromGaps: 1
...
const GOVERNANCE_RESULT = { projectId: "project_linked", governancePosture: "audit_only", externalPushPolicy: "external pushes observed · tanren opens no PRs" };
...
function mockOrchestrator(): void {
  vi.stubGlobal("fetch", async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (/\/orgs\/[^/]+\/projects$/.test(url) && method === "GET") return new Response(JSON.stringify({ projects: [PROJECT] }), { status: 200 });
    if (/\/orgs\/[^/]+\/projects$/.test(url) && method === "POST") return new Response(JSON.stringify(PROJECT), { status: 201 });
    if (url.endsWith("/link") && method === "POST") return new Response(JSON.stringify({ projectId: PROJECT.projectId, repoUrl: PROJECT.repoUrl, orgId: ORG.id, detectedFiles: [], writesPerformed: 0 }), { status: 200 });
    if (url.endsWith("/recon") && method === "POST") return new Response(JSON.stringify(RECON_RESULT), { status: 200 });
    if (url.endsWith("/config-injection") && method === "POST") return new Response(JSON.stringify(CONFIG_INJECTION_RESULT), { status: 201 });
    if (url.endsWith("/seed-dag") && method === "POST") return new Response(JSON.stringify(SEED_RESULT), { status: 201 });
    if (url.endsWith("/governance") && method === "POST") return new Response(JSON.stringify(GOVERNANCE_RESULT), { status: 200 });
...
describe("brownfield · config-injection PR (step 3)", () => {
  it("renders the 6-file preview with per-file exclude checkboxes", async () => {
    ...
    const res = await app.request("/onboarding/existing", {
      method: "POST",
      headers: FORM,
      body: new URLSearchParams({ phase: "advance", step: "2", repoUrl: PROJECT.repoUrl, projectId: PROJECT.projectId, report: REPORT_JSON })
    });
    const html = await res.text();
...
describe("brownfield · DAG seed (step 4)", () => {
  it("seed POST creates specs from issues + gaps and renders the source legend", async () => {
    ...
    const res = await app.request("/onboarding/existing", {
      method: "POST",
      headers: FORM,
      body: new URLSearchParams({ phase: "seed", step: "4", repoUrl: PROJECT.repoUrl, projectId: PROJECT.projectId, report: REPORT_JSON })
    });
    const html = await res.text();
...
describe("brownfield · governance (step 5)", () => {
  it("governance POST persists the posture + renders the external-push policy", async () => {
    ...
    const res = await app.request("/onboarding/existing", {
      method: "POST",
      headers: FORM,
      body: new URLSearchParams({ phase: "governance", step: "5", repoUrl: PROJECT.repoUrl, projectId: PROJECT.projectId, posture: "audit_only" })
    });
    const html = await res.text();
  ...
  it("renders all 3 posture options on the picker", async () => {
    ...
    const res = await app.request("/onboarding/existing", {
      method: "POST",
      headers: FORM,
      body: new URLSearchParams({ phase: "advance", step: "4", repoUrl: PROJECT.repoUrl, projectId: PROJECT.projectId, report: REPORT_JSON })
    });
    const html = await res.text();
