...
//     prefixes them with a CAPTURED, CHECKED, then evaluated `mise activate bash --shims`
//     (the documented non-interactive / CI activation — see `miseActivationPrelude` for
//     WHY hook-mode is wrong here, and for why the activation is NOT the obvious
//     `eval "$(…)"`), guarded on a `mise.toml` being present in the cwd (so a project
