...
//
// LOUD ON A FAILED ACTIVATION (branch 1): `eval "$(mise activate …)"` reports the status
// of EVAL, never of the command substitution inside it — a `mise activate` that exits
// nonzero evaluates to an empty string and "succeeds", so the command runs with the
// project's toolchain absent and the only evidence is a line on stderr. That is the silent
// degradation this module exists to remove, sitting in the module itself. The guard has
// already established the repo ships a `mise.toml`, so a failed activation means the
// declared toolchain is not on PATH; we CAPTURE, CHECK, then evaluate, and halt loud rather
// than run the project's own command against whatever the image happens to carry. The guard
// stays a SKIP for a repo that declared nothing — that path never enters this branch.
 ...
 *
 * `workspacePath` is REQUIRED and threaded straight into {@link withMiseActivation}: the
 * hook activation must use THIS run's per-run mise scope ({@link miseRunScope}), never a
 * runner-wide one a concurrent run would collide on.
 */
