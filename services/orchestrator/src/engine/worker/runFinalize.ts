 ...
 */
export async function finalizeRunQuotaExceeded(
  pool: pg.Pool,
  writer: RunStateWriter | undefined,
  runId: string,
  reason: string,
  ...
  // Plane-split P3: route the deny-path finalize + run.quota_exceeded event
  // through the control plane when a remote writer is wired (the gate's call site
  // always has an org). Same guard, same event, only server-side.
  if (writer !== undefined && orgId !== null) {
    const result = await writer.finalizeRun({
      runId,
      orgId,
