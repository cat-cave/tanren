...
/** Typed provider failure so workflow disposition can distinguish transient host faults. */
export class GitHubReviewHttpError extends Error {
  readonly retriable: boolean;
  constructor(
    operation: "list" | "submit",
    readonly status: number,
    detail?: string,
  ) {
    const label = operation === "submit" ? "submit-review" : "PR reviews list";
    super(`GitHub ${label} failed: ${detail ?? `HTTP ${status}`}`);
    this.name = "GitHubReviewHttpError";
...
export class GitHubReviewMergeService {
  ...
  async listPullRequestReviews(
    ...
    for (;;) {
      ...
      if (response.status !== 200) {
        throw new GitHubReviewHttpError("list", response.status);
      }
  ...
  async submitReview(
    ...
    if (response.status !== 200 && response.status !== 201) {
      ...
      throw new GitHubReviewHttpError("submit", response.status, message);
    }
