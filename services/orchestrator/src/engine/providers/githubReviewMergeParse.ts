...
export interface ParsedSubmitReviewReceipt {
  ...
  headSha: string;
  /** Provider user.login — required for land-authoritative simulated receipt. */
...
export function receiptFromListedReview(review: ListedPullRequestReview): ParsedSubmitReviewReceipt {
  ...
  if (review.reviewerLogin === undefined || review.reviewerLogin === "") {
    throw new TypeError(`listed review ${review.forgeReviewId} missing provider reviewer login`);
  }
  return {
...
function parseAuthoritativeReviewObject(object: Record<string, unknown>, label: string): ParsedSubmitReviewReceipt {
  ...
  const reviewerLogin = reviewerLoginFrom(object["user"]);
  if (reviewerLogin === undefined || reviewerLogin === "") {
    throw new TypeError(`GitHub ${label} carried no provider reviewer login`);
  }
  return {
    forgeReviewId,
    forgeReviewState,
