...
async function commitWorkspaceChangesAfterCodex(
  ...
  // Staging (and the staged-change probe it carries) runs as its own command so ONLY the
  // commit's exit can be read as a hook verdict — an `index.lock` conflict, a permission
  // error, or a `git diff --cached` probe fault (exit > 1) is a substrate fault and must keep
  // throwing, not be re-told to the writer as "your work was rejected". See
  // `stageWorkspaceChanges`.
  const hasStagedChanges = await stageWorkspaceChanges(ssh, target, workspace, "stage codex workspace changes");
  ...
  // loop re-drives the writer with it. The gated command is the bare commit ALONE — no
  // second fallible step — so only the hook's verdict can be read from its exit. A substrate
  // failure or watchdog stall still throws.
