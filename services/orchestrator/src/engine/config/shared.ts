...
// `contracts/auditPosture.ts`). Expressed as Zod here so the per-project
// `auditPosture` config persists + validates with the rest of the project blob.
...
// The per-project AUDIT POSTURE — the REAL DORA knob (tanren-owns-the-engine.md
// §4). The auditor emits findings; this posture turns them into the gate verdict.
// `blockReviewAt` is the severity at-or-above which a finding BLOCKS reaching
// review/merge; `p2p3Handling` decides what happens to the below-threshold
// residual. It is a GOVERNED SETTING (exactly like `governancePosture` /
// `reviewPolicy`), persisted in the project config jsonb — never an env var.
//
// The default is the BALANCED posture: `blockReviewAt: 'P1'` (P0/P1 block; P2/P3
// do not) with `p2p3Handling: 'fix-if-idle'`. A zero-defect shop sets
// `blockReviewAt: 'P3'` (block on anything); a velocity shop sets
// `blockReviewAt: 'P1', p2p3Handling: 'route-to-dag'` (block only P0/P1, route the
// rest). One engine, every strategy — DORA metrics let a user MEASURE which fits.
...
// The balanced default (P0/P1 block, residual fixed in place when idle) — the
