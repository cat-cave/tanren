 ...
 * + ancestor head SHAs the run row records, via the pure `memberKey`. WRAPPED in
 * try/catch-log: a node-write bug can NEVER fail a run (no reads drive behavior;
 * this is pure observation written alongside the existing model). A non-speculative
 * run (no base, no ancestors) is a node with ZERO members — still a node (the
 * unified model: one run, base may shift), so it is recorded too.
 *
...
export async function observeRunAsIntegrationNode(
  ...
  try {
    ...
    // A null-org (CLI) run has no tenant key to scope a write to — skip it.
    ...
    // OBSERVE-ONLY: a node-write failure must NEVER fail a run. Log loudly + move on.
    console.warn(
