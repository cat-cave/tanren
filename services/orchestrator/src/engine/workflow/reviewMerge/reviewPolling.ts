...
import { GitHubReviewMergeService, type ReviewVerdict, type ReviewVerdictResult } from "../../providers/githubReviewMerge.js";
...
async function finalizeReviewTask(
  ...
  const base = { runId: context.runId, specId: context.specId, projectId: context.projectId, taskId: result.taskId };
  if (result.verdict === "approved") {
    await pool.query("UPDATE tasks SET status = 'done', outcome = 'ok', ended_at = now() WHERE task_id = $1", [result.taskId]);
    await eventStore.append({
      ...base,
      eventType: "review.approved",
      payload: { prUrl: result.prUrl, prNumber: result.prNumber, reviewer: result.reviewer }
    });
    await eventStore.append({ ...base, eventType: "task.completed", payload: { taskKind: "review", status: "approved" } });
    return;
  }
  if (result.verdict === "changes_requested") {
    ...
    await eventStore.append({
      ...
      payload: { prUrl: result.prUrl, prNumber: result.prNumber, reviewer: result.reviewer, message: result.feedback }
    });
    await eventStore.append({ ...base, eventType: "task.completed", payload: { taskKind: "review", status: "changes_requested" } });
    return;
  }
  // Pending after the budget: leave the task running (the operator hand-off and
  // the dashboard surface drive it from here).
  await pool.query("UPDATE tasks SET status = 'running', outcome = 'pending', ended_at = NULL WHERE task_id = $1", [result.taskId]);
}
...
async function buildGitHubProbe(
  ...
  return {
    markReady: () => service.markReadyForReview({ repo, pullNumber, token: resolved.token, refreshToken: resolved.refresh }),
    fetchVerdict: () => service.fetchReviewVerdict({ repo, pullNumber, token: resolved.token, refreshToken: resolved.refresh })
  };
}
...
async function ensureReviewTask(pool: RunStateClient, context: ReviewMergeRunContext): Promise<string> {
  ...
  await pool.query(
    `INSERT INTO tasks (task_id, run_id, kind, title, status, started_at, agent_kind, cli, model, attempt)
     VALUES ($1, $2, 'review', 'Poll pull request review', 'running', now(), 'system', 'github', NULL, 1)`,
