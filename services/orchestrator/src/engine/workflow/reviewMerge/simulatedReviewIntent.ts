...
import { ReviewSimulatedIntentPayload } from "../../events/schemas/integrations.js";
...
export interface SimulatedReviewIntentRepository {
  lookup(runId: string, headSha: string): Promise<SimulatedReviewIntent | undefined>;
  /**
...
export class PgSimulatedReviewIntentRepository implements SimulatedReviewIntentRepository {
  ...
  async lookup(runId: string, headSha: string): Promise<SimulatedReviewIntent | undefined> {
    ...
    const result = await client.query<{ payload: unknown }>(
      `SELECT payload FROM events
        ...
        LIMIT 1`,
      [runId, REVIEW_SIMULATED_INTENT_EVENT, key],
    );
  ...
  async adoptOrRecord(input: {
    ...
    try {
      ...
      const existingAfterRace = await this.lookup(input.runId, headSha);
    ...
    const winner = await this.lookup(input.runId, headSha);
    if (winner === undefined) {
