...
import {
  ...
  appendRoleFailed,
  appendRoleStarted,
  failureForFixtureTask,
  recordFixtureCost,
  secondsSince,
  type Phase1TaskKind,
} from "./phase1FixtureRoles.js";
...
export async function runPhase1FixtureWorkflow(input: RunPhase1FixtureInput): Promise<Phase1FixtureResult> {
  ...
  await appendEvent("phase1.fixture.started", {
  ...
  try {
    ...
    const writer = await runFixtureTask(
      input.pool,
      appendEvent,
      context.runId,
      "write",
      "Writer fixture change",
      "writer",
      writerAdapter.cli,
      async (taskId) => {
        ...
        await appendEvent("workspace.git_captured", {
          workspacePath,
    ...
    const check = await runFixtureTask(
      input.pool,
      appendEvent,
      context.runId,
      "check",
      "Check writer fixture output",
      "answerer",
      checker.cli,
      async (taskId) => {
    ...
    const audit = await runFixtureTask(
      input.pool,
      appendEvent,
      context.runId,
      "audit",
      "Audit phase 1 fixture",
      "answerer",
      auditor.cli,
      async (taskId) => {
    ...
    await input.pool.query(
      "UPDATE runs SET status = 'done', outcome = 'phase1_fixture_complete', ended_at = now() WHERE run_id = $1",
      [context.runId],
    );
    await appendEvent("phase1.fixture.completed", {
...
async function pollCiUntilTerminal(
  ...
  for (let attempt = 0; attempt < maxPolls; attempt += 1) {
    ...
    if (attempt < maxPolls - 1) {
      await appendEvent("phase1.fixture.ci_pending", {
        attempt: attempt + 1,
        nextPollAfterMs: delayMs,
      });
      await sleep(delayMs);
    }
  }
