...
export async function createScratchCopy(deps: ScratchCopyDeps, sourcePath: string, scratchPath: string): Promise<void> {
  ...
  const result = await deps.ssh.run(deps.target, { command, timeoutMs: deps.timeoutMs });
...
export async function removeScratchCopy(deps: ScratchCopyDeps, scratchPath: string): Promise<{ removed: boolean }> {
  ...
  return { removed: result.failure === undefined && !result.timedOut && result.exitCode === 0 };
}

function assertOk(
  result: { exitCode: number | null; timedOut: boolean; stderr: string; failure?: unknown },
  doing: string,
): void {
  ...
  if (result.timedOut) {
    throw new ScratchCopyError(`${doing}: timed out`);
  }
