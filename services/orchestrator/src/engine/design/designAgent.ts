...
export function buildDesignAgentPrompt(input: DesignAgentInput): string {
  ...
  return [
    ...
    "behavior the design is responsible for. EVERY behavior MUST be covered by exactly",
    "one designed surface/flow in your `coverage` array, attributed to that behavior's",
    "persona and tied to one of your dimensions. A behavior with NO covering surface is",
    "an incomplete contract and will be REJECTED. Do not invent behaviors; cover the",
    "ones provided.",
    "",
    ...
    "- `coverage`: ONE entry per behavior above — `behaviorId` (a provided behavior id),",
    "  `personaId` (that behavior's persona), `dimensionKey` (one of your dimensions),",
    "  and `surface` (the designed surface/flow that covers it). Cover EVERY behavior.",
  ].join("\n");
}
