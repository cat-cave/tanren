...
const SCRIPT: InterviewRoundOutput[] = [
  {
    ...
    captureDelta: {},
    suggestions: [{ label: "a supply-chain ops tool", value: "supply chain operations for mid-market manufacturers" }],
    complete: false
  },
  {
  ...
  },
  {
    ...
    captureDelta: {
      behaviors: [
        { persona: "line worker", title: "clock in with badge", given: "line worker at handheld", when: "they scan their badge", then: "a shift opens + pick list loads" },
        { persona: "line worker", title: "pull the day's pick list", given: "an open shift", when: "they open the station", then: "the pick list for their station loads" },
        { persona: "line worker", title: "scan items off the bin", given: "a pick list", when: "they scan an item", then: "the item is added to the tote" },
        { persona: "line worker", title: "confirm each scan", given: "a scanned item", when: "they confirm", then: "the scan is committed" },
        { persona: "line worker", title: "push tote to staging", given: "a full tote", when: "they push it", then: "the tote moves to staging" }
      ]
    },
    suggestions: [],
    complete: false
  },
  {
    ...
    captureDelta: {
      behaviors: [
        { persona: "ops manager", title: "place a purchase order", given: "an ops manager", when: "they submit a PO", then: "the order is created + routed" },
        { persona: "ops manager", title: "review supplier scorecard", given: "a supplier", when: "they open its scorecard", then: "scoring + history render" }
      ]
    },
    suggestions: [],
    complete: false
  },
  {
    ...
    captureDelta: {
      behaviors: [{ persona: "cfo", title: "run the monthly close", given: "a month end", when: "they run the close", then: "variance + exports are produced" }]
    },
    suggestions: [{ label: "desktop + offline handheld", value: "desktop dashboard for ops + cfo, offline-capable handheld for the line worker" }],
    complete: false
  },
  {
  ...
  },
  {
    ...
    captureDelta: {
      rulesets: ["main protected · pr required", "linear history", "status checks required", "no force push", "mergify on"]
    },
    suggestions: [],
    complete: true
  }
