// P3-0022 candidate-inbox store: the persistence seam over the `inbox_sources`
// + `candidates` tables (migration 0024). Pure SQL + row mapping; no Forge or
...
export async function createSource(client: QueryClient, input: CreateSourceInput): Promise<InboxSource> {
...
export async function listSources(client: QueryClient, orgId: string): Promise<InboxSource[]> {
  const result = await client.query<SourceRow>(
...
export async function getSource(client: QueryClient, sourceId: string): Promise<InboxSource | undefined> {
  const result = await client.query<SourceRow>(
    `SELECT id, org_id, project_id, kind, name, detail, config, enabled, auto_route
...
export async function listCandidates(client: QueryClient, orgId: string): Promise<Candidate[]> {
  const result = await client.query<CandidateRow>(
...
export async function getCandidate(client: QueryClient, candidateId: string): Promise<Candidate | undefined> {
  const result = await client.query<CandidateRow>(
    `SELECT c.id, c.source_id, c.org_id, c.project_id, c.external_id, c.title, c.body,
            c.severity, c.status, c.triage, c.resolved_spec_id,
            s.name AS source_name, s.kind AS source_kind
