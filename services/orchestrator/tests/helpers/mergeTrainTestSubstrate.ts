...
export class TestProofSubstrate implements ProofSubstrate {
  ...
  async seal(input: { proofRoot: Digest; bindings: BundleBindings }) {
    return { signingKeyId: MQ15_TEST_HMAC_KEY, rootSignature: this.sign(input.proofRoot) };
  }

  private sign(proofRoot: Digest): Uint8Array {
    return new Uint8Array(createHmac("sha256", MQ15_TEST_HMAC_KEY).update(proofRoot).digest());
  }
  ...
  async constructBundle(input: {
    ...
    const bundleBytes = new TextEncoder().encode(canonicalJson({ proofRoot, nonce: input.bindings.nonce }));
    ...
    return {
      bundleId,
      bundleDigest: this.digestOf({ bundleId, proofRoot, bytesDigest: bytesRef.digest }),
      proofRoot,
      ...
      rootSignature: this.sign(proofRoot),
    };
  }

  async verify(bundle: ProofBundleSealed) {
    const expected = Buffer.from(this.sign(bundle.proofRoot));
