...
describe("template-fragment isolation — every registered fragment composes against the minimal base", () => {
  for (const fragment of loadFragmentLibrary().all()) {
    it(`fragment "${fragment.id}" composes alone + preserves its declared contract`, async () => {
      ...
      // apex v63 ERR_PNPM_NO_LOCKFILE halt class). The scope covers BOTH the
      // justfile bootstrap recipe AND any Dockerfile RUN line (audit finding
      // #10). Per-fragment isolation surfaces a single-fragment regression (e.g.
      // a future runtime that ships an unflagged `npm ci`, or an addon-docker
