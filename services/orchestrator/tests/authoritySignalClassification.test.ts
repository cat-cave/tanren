...
import { MergeSignalClassifiedPayload } from "../src/engine/events/schemas/mergeQueueAuthoritySignals.js";
import { mergeQueueAuthoritySignalSensitivityRules } from "../src/engine/events/sensitivityRules.mergeQueueAuthoritySignals.js";
import {
...
describe("mq-1 typed authority-signal classification", () => {
  ...
  it("declares every payload field public while excluding prose and raw source keys", () => {
    const paths = new Set(mergeQueueAuthoritySignalSensitivityRules.map((rule) => rule.path));
    ...
    expect(paths.has("body")).toBe(false);
    expect(paths.has("sourceKey")).toBe(false);
  });
});
