...
// Generic sha256 digest.
const D = `sha256:${"c".repeat(64)}`;
// Per-project CAS artifact / proof-unit digest.
const CAS_A = `sha256:${"a".repeat(64)}`;
...
async function seedLineage(
  ...
  await owner.query(
    ...
    [org, id("verification_run"), project, id("venv"), D, casDigest],
  );
  await owner.query(
    `WITH verdict AS (
    ...
    [org, L.verdict, project, id("verification_run"), L.behaviorRevision, D, casDigest],
  );
  // authority decision.
  await owner.query(
