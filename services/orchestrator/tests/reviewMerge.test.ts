...
describe("merge dispatch stage", () => {
  ...
  it("P2a: branch BEHIND → updateBranch → re-gate CI green → merge.behind + merge.rebased + merge.completed", async () => {
    const pool = new ReviewMergePool("direct_merge");
    const events = new FakeEventStore();
    const probe = recordingMergeProbe(
  ...
  it("P2a: update-branch reports a CONFLICT (422/dirty) → routed to conflict hook, merge.conflict, NO merge", async () => {
    const pool = new ReviewMergePool("direct_merge");
    const events = new FakeEventStore();
    const probe = recordingMergeProbe(
  ...
  it("P2a: branch DIRTY (real conflict with base) → routed to hook BEFORE any merge call", async () => {
    const pool = new ReviewMergePool("direct_merge");
    const events = new FakeEventStore();
    const probe = recordingMergeProbe(
    ...
    expect(hookCalls).toBe(1);
    expect(probe.mergeCalls).toBe(0);
    expect(probe.updateBranchCalls).toBe(0); // dirty: no point trying update-branch.
    expect(events.events.map((e) => e.eventType)).toContain("merge.conflict");
  });

  it("P2a: re-gated CI FAILS after rebase → merge.failed, NO merge", async () => {
    const pool = new ReviewMergePool("direct_merge");
    const events = new FakeEventStore();
    const probe = recordingMergeProbe(
      { merged: true, conflict: false, status: 200, message: "merged" },
  ...
  it("P2a: a CLEAN branch merges directly with no update-branch call", async () => {
    const pool = new ReviewMergePool("direct_merge");
    const events = new FakeEventStore();
    const probe = recordingMergeProbe({
      merged: true,
