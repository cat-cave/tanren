...
describe("DesignContractStore.getLatestState — typed lookup (Codex critic #7)", () => {
  ...
  // H2 BLOCKING (unify): the store keys the head on `(project_id, version)`
  // alone — one contract per project, shared across every spec type. Prove the
  // SELECT does NOT include a `mode = $N` filter (migration 0028 dropped it):
  // a query spy captures the SQL text and any bound params issued.
  it("H2: getLatestState filters by project_id only (no mode filter)", async () => {
    const contract = contractFixture();
    let capturedSql = "";
    ...
    const spy = {
      async query(sql: unknown, params: readonly unknown[] = []): Promise<{ rows: unknown[]; rowCount: number }> {
        ...
        if (capturedSql.includes("FROM design_contracts")) {
          return {
            rows: [
              {
                id: "design_1",
    ...
    const state = await DesignContractStore.getLatestState(spy, PROJECT, ACTOR);
    expect(state.kind).toBe("found");
    // The SQL keys on project_id ONLY — the collapsed mode-keying is gone.
    expect(capturedSql).toContain("project_id = $1");
    expect(capturedSql).not.toContain("mode");
