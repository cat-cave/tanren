...
// This suite spawns real `jj` + real `git` subprocesses per case (see fixture builders
// below). Under the 48-thread vitest pool the subprocess contention races the default
// 5s per-test timeout even though each op completes fine in isolation — silent flake
// in `just fast-check` / pre-push. Raise the ceiling for this file so the suite stays
// a reliable gate. Task #25.
