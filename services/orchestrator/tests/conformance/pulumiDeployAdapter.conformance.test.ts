...
const METADATA = { pulumiBackend: "https://api.pulumi.com", pulumiProject: "acme-infra" };
...
describe("PulumiDeployAdapter — lifecycle", () => {
  ...
  it("provisionOrBind(bind) links an already-discovered stack name", async () => {
    ...
    const artifact = await instance.provisionOrBind(
      await operationGrant("bind", projectIntegrationOperationTarget(projectCtx, "prod-stack"), METADATA, projectCtx),
      projectCtx,
      {
        mode: "bind",
...
describe("PulumiDeployAdapter — loud fail on missing config", () => {
  ...
  it("throws a typed config error when the project is absent", async () => {
    const { instance } = adapter();
    const noProject = await operationGrant(
      "deploy",
      { resourceId: ref.appId, sourceRepo: source.repo, sourceRef: source.ref },
      { pulumiBackend: "https://api.pulumi.com" },
    );
