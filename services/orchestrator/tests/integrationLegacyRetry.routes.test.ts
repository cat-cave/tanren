...
describe("legacy Sentry route retries", () => {
  it("keeps an endpoint-less Sentry request retry-only when no legacy row exists", async () => {
    ...
    const response = await app.request("/orgs/org_acme/integrations/sentry", {
      ...
      body: JSON.stringify({ token: "new-token", idempotencyKey: "missing-legacy" }),
    });

    expect(response.status).toBe(400);
    expect(await response.json()).toMatchObject({ error: "invalid_provider_endpoint" });
  ...
  it("does not migrate a historical v1 row when the Sentry endpoint changes", async () => {
    const token = "endpoint-bound-token";
    const fetchImpl = vi.fn<typeof fetch>(async () =>
      sentryOrganizationsResponse([{ id: "org-a", slug: "a", name: "A" }]),
    ) as unknown as typeof fetch;
    const { app, database } = harness({ actor: admin, fetchImpl });
    const first = await app.request("/orgs/org_acme/integrations/sentry", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token, idempotencyKey: "endpoint-change", baseUrl: "https://sentry.io" }),
    });
    expect(first.status).toBe(202);
    const operation = database.memory.operations[0];
    operation.request_fingerprint = legacyIntegrationRequestFingerprint({
    ...
    const changed = await app.request("/orgs/org_acme/integrations/sentry", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token, idempotencyKey: "endpoint-change", baseUrl: "https://sentry.example" }),
    });
  ...
  it("migrates a historical Sentry rotation through the bound endpoint path", async () => {
    const endpoint = "https://sentry.io";
    const fetchImpl = vi.fn<typeof fetch>(async () =>
      sentryOrganizationsResponse([{ id: "org-a", slug: "a", name: "A" }], endpoint),
    ) as unknown as typeof fetch;
    ...
    const linked = await app.request("/orgs/org_acme/integrations/sentry", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token: "link-token", idempotencyKey: "bound-rotation-link", baseUrl: endpoint }),
    });
    expect(linked.status).toBe(202);
    const { connectionId } = (await linked.json()) as { connectionId: string };
    const first = await app.request(`/orgs/org_acme/integrations/sentry/connections/${connectionId}/rotate`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token: "rotation-token", idempotencyKey: "bound-rotation" }),
    });
    expect(first.status).toBe(202);
    const operation = database.memory.operations.find((row) => row.idempotency_key === "bound-rotation");
    expect(operation).toBeDefined();
    operation!.request_fingerprint = legacyIntegrationRequestFingerprint({
      orgId: "org_acme",
      providerKind: "sentry",
