...
// unscoped reads/writes, while the repository also preserves project edge
// boundaries within one org. Gated like the existing RLS cohort.
...
const ACTOR = { kind: "operator" as const, id: "rv4-proof" };
...
describeDb("RV4-BEHAVIOR-COVERAGE-RLS — org and project isolation", () => {
  ...
  it("round-trips canonical edges under the exact org/project scope", async () => {
    await runWithOrgScope(runtimePool, ORG_A, async (client) => {
      ...
      const snapshot = await BehaviorCoverageEdgesStore.readSnapshot(
        client,
        { orgId: ORG_A, projectId: PROJECT_A },
        ACTOR,
      );
  ...
  it("does not leak project edges into a second project in the same org", async () => {
    const snapshot = await runWithOrgScope(runtimePool, ORG_A, (client) =>
      BehaviorCoverageEdgesStore.readSnapshot(client, { orgId: ORG_A, projectId: PROJECT_A2 }, ACTOR),
    );
  ...
  it("denies cross-org and unscoped reads instead of returning foreign facts", async () => {
    const crossOrg = await runWithOrgScope(runtimePool, ORG_A, (client) =>
      BehaviorCoverageEdgesStore.readSnapshot(client, { orgId: ORG_B, projectId: PROJECT_B }, ACTOR),
    );
    ...
    const unscoped = await BehaviorCoverageEdgesStore.readSnapshot(
      runtimePool,
      { orgId: ORG_A, projectId: PROJECT_A },
      ACTOR,
    );
