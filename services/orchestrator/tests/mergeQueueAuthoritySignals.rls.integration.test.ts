...
const EVALUATION = "mqeval_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
...
describeDb("mq-1 authority-signal route under enforced event RLS", () => {
  ...
  beforeAll(async () => {
    ...
    // Catalog rows come from EV-SUB-W0 migration 0042 via migrate().
  ...
  await runWithOrgScope(owner, orgId, () =>
    new PgEventStore(owner).append({
      ...
      payload: {
        ...
        groupId: "mqgrp_bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
        memberIds: [memberId],
