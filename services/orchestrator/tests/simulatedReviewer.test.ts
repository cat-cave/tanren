...
describe("review polling stage — reviewPolicy: simulated (gv-2 strict publication)", () => {
    ...
    expect(captured.submitted).toEqual([
      {
        ...
        body: expect.stringMatching(
          /Tanren simulated review.*tanren-simulated-review:v1:approved.*tanren-simulated-review-intent:v1:[0-9a-f]{64}/su,
        ),
        headSha: HEAD,
      },
    ]);
    ...
    expect(captured.submitted).toEqual([
      {
        ...
        body: expect.stringMatching(
          /Tanren simulated review.*tanren-simulated-review:v1:changes_requested.*tanren-simulated-review-intent:v1:[0-9a-f]{64}/su,
        ),
        headSha: HEAD,
      },
    ]);
  ...
  it("throws when the probe cannot pin a publisher — no internal-only authority", async () => {
    ...
    ).rejects.toThrow(/strict review|publication/iu);
