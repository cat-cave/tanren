...
describeLive("live Codex writer adapter", () => {
  it("materializes managed auth and makes a tiny mutation in a runner workspace", async () => {
    ...
    await storeCodexAuthBundle(secrets, { ref: "credential/codex/live", authJson: await readFile(authPath, "utf8") });
    ...
    const writer = createCodexWriter({ secrets, ssh, target, credentialRef: "credential/codex/live", runId });
