...
| **Gate / CI**         | The native gate runs on Tanren's own runners (no GitHub Actions minutes) — its cost is the runner compute above, not a separate forge-CI line item. | **No** — folded into Compute / VM.                                                                            |
