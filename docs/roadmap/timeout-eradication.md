...
| `services/allocator/src/runnerLifecycle.ts:383` (`sweepStuck(unclaimedGraceMs)`) — the wall-clock `TANREN_MAX_RUN_HOURS` reaper is DELETED (no such env in the allocator)                                                                                                                      | no wall-clock age setting                                    | already eradicated — reclaims ONLY terminal-run, lease-lapsed, or unclaimed-grace runners; a healthy in-flight runner is never reaped by age. | DONE — sign-of-life (no lease renewal = dead)                                                    |
...
- **M5 — the tail.** allocator age-based reaping is already removed; retain
  sign-of-life reclamation. The 7 forge answerer `timeoutMs ?? 120/180k`; the
