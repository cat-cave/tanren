...
- **Workflow completion** _(built)_: real review polling (ready-for-review marking, changes-requested handling), real merge contract with per-repo configurable integrations (Mergify queue · direct GitHub merge · external-reviewer handoff), conflict resolver scaffolding, optional Mergify merge-queue integration.
