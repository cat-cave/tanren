...
| `0040_governance_coverage.sql`   | `coverage_requirements`, `coverage_observations`, `coverage_artifacts`, `runtime_verification_runs`, `runtime_scenarios`, `negative_control_results`                                                                                 |
...
- `POST .../coverage/runtime-verification-runs`
- `GET .../coverage/runtime-verification-runs/:runId`
...
- `GET .../stacks/:specId`
- `POST .../stacks/:specId/exercise` — privileged runtime-verification exercise
- `GET .../base-shifts`
...
- A one-click privileged depth-N runtime-verification exercise.
...
All events must flow through `PgEventStore`; no subsystem gets a second audit stream.
Each runtime-verification run carries `runtimeVerificationRunId`, correlation ID,
org/project, policy revision/hash, and environment identity.
...
| **2. F2/F3/F4 first-class control**            | Atomic audit lineage/materialization, integration nodes authoritative, depth-6 general DAG validation, reservations/infra costs/pause episodes, budget UI.                                                               | 22–32 engineer-weeks |
...
| **5. Coverage/DORA/live-validation hardening** | Coverage ledger, signed artifacts, negative controls, revision-segmented DORA, production rollout/migration/drift reconciliation.                                                                                        | 20–30 engineer-weeks |
...
- Governance Studio, coverage ledger, and live F1–F5 runtime-verification scenarios.
...
- **Product decisions still needed:** default tier for brownfield imports, maximum tolerated stale coverage, distinct-person requirements across categories, supported reauthentication providers, receipt semantics per channel, Fly promotion/rollback behavior, and whether stale required live-validation evidence automatically pauses new work or only blocks policy activation.
