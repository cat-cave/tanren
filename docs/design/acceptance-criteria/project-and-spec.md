...
- [ ] **Project detail tabs**: settings (routing & limits — see `routing-and-limits.md`), governance (read-only display of detected `.github/workflows/`, `.mergify.yml`, `CODEOWNERS` from the linked repo), notifications (per-project overrides on org defaults), behaviors/milestones (CRUD against P2A-0018).
