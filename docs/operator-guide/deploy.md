...
| `TANREN_RUNNER_IDENTITY_KEY_PATH`   | Path, mounted into the orchestrator + worker, to the PRIVATE key file the orchestrator SSHes into runners with. Delivered as a mounted secret FILE (like the mTLS keys), never as an env value, so the key never transits Docker env / `docker inspect`. |
