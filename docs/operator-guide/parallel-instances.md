...
from *in development* to *in beta*. You have never seen the internals. This page
...
- [`apex.md`](./apex.md) — what a max-difficulty run *is* and the run rhythm.
...
Parallel operation is **not** a feature *inside* Tanren. There is no "swarm mode",
...
link-shortener-with-Slack used in earlier trials is just *one* example). The whole
...
and deploy-target keying) hold *even within a single install that hosts multiple
orgs*, so they are the backbone of isolation whether operators are on one machine
or many.
...
> a property of *the org*, not *the machine*. Separate machines are the simplest
...
2. **Adversarial cross-model review.** A reviewer tries to *refute* the change's
...
You *can* run more than one project on a single machine, but only if the **runner
...
every allocation the *same* single dev-compose `runner` container (it is explicitly
...
Tanren today is *in development*: the autonomy engine is merged and the operator
...
*In beta* is the point at which a newcomer can, from these docs alone:
