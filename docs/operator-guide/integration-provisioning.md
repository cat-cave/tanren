...
worthwhile per provider — not missing foundations.

| Integration               | Correct org-level grant                                                                         | Project/resource artifact Tanren should manage                               | Remaining refinement                                                                                                                                                                                                                                                       |
| ------------------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Generic webhook           | Endpoint base/receiver capability owned by operator or Tanren                                   | Per-project URL/signing secret subscription                                  | Outbound POST ships. Add outbound signing + a webhook destination provision/test flow.                                                                                                                                                                                     |
| Email                     | Domain/provider API key, verified sender/domain                                                 | Sender identity, recipient targets, templates                                | Delivery ships. Treat the provider key/domain as the org grant; provision sender/domain where APIs permit; project targets stay config.                                                                                                                                    |
| PagerDuty                 | Account API token or service integration permission                                             | Service/integration routing key                                              | Routing-key delivery ships. Add a provisioner to create/find the service integration and store the routing key; keep the direct key as fallback.                                                                                                                           |
| Twilio                    | Account SID/auth token with messaging permissions                                               | Messaging service/phone number/sender policy and recipient target            | Delivery ships (SID/token/from). Add provisioning/discovery for the messaging service/from number; A2P compliance stays an account-level prerequisite.                                                                                                                     |
| ntfy                      | Optional server/topic auth                                                                      | Topic                                                                        | Simple topic delivery ships. For private topics, store topic/token as a managed artifact.                                                                                                                                                                                  |
| GitHub checks             | Reuse GitHub App                                                                                | PR head status context                                                       | Correctly reuses GitHub auth; no new credential.                                                                                                                                                                                                                           |
| Hetzner allocator         | Hetzner project API token                                                                       | Per-run SSH key, cloud-init/user data, host identity strategy, server labels | **Shipped** (`hetznerAllocator`): ephemeral SSH key + cloud-init + pinned host key. Refine: image-baked host-key strategy.                                                                                                                                                 |
| DigitalOcean allocator    | DO project token                                                                                | SSH key/fingerprint, droplet tags, cloud-init                                | Allocator is credential+config driven. Add the same provisioner pattern as Hetzner (ephemeral key + pinned host key).                                                                                                                                                      |
| AWS EC2 allocator         | IAM role/access key scoped to EC2/VPC resources                                                 | Key pair, security group/subnet/AMI/user-data/host key                       | Allocator is config driven. Add an environment provisioner or split account bootstrap from per-project provisioning; prefer short-lived role creds.                                                                                                                        |
| GCP allocator             | Service account / workload identity                                                             | Project/zone/network/metadata startup script/host key                        | Allocator is credential+config driven. Add provisioner/discovery for network/instance template/startup script.                                                                                                                                                             |
| Kubernetes allocator      | Cluster/admin or namespace-scoped credential                                                    | Namespace, service account, runner pod/image pull config                     | Config expects namespace/token. Add a namespace-bootstrap provisioner for SaaS/self-hosted clusters.                                                                                                                                                                       |
| Deploy target             | Provider org/team token                                                                         | Project/app/site, env vars, preview URL pattern, deployment hook             | **Shipped** for **Vercel + Fly** (`provisioners/{vercel,fly}DeployProvisioner.ts`): create app/project, attach env vars, store preview URL + deploy refs. Per-provider provisioner metadata (Vercel `teamId`, Fly `orgSlug`) is documented in `apex-run-playbook.md` §4.b. |
| OpenRouter/managed router | Platform/org API key                                                                            | Per-project route/model/budget policy                                        | Org/platform credential ships. Keep org-level; project config only selects mode/model policy/budget.                                                                                                                                                                       |
| Secret stores             | Infra bootstrap credential                                                                      | Secret paths/namespaces                                                      | Correctly bootstrap-level (not per-project). Provide migration/import tooling.                                                                                                                                                                                             |
| GitHub OAuth / OIDC       | Deployment/client config or per-org IdP app                                                     | Redirect URIs/group mapping                                                  | Infra-env today. For SaaS the platform owns the app (org mapping is config); for self-hosted the operator creates the IdP app once.                                                                                                                                        |
