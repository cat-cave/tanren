...
`just smoke` builds the images, starts the compose stack, runs the connectivity check (`tanren doctor` — orchestrator `/healthz` plus direct runner SSH), and runs the live SSH integration test. (The synthetic `hello` fake-agent workflow has been purged from the runtime.)
