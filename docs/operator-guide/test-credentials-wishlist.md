...
| **Hetzner**      | Project API token only — Tanren auto-manages the ephemeral SSH key + host key (P-INT-5)                                                       | `TANREN_HETZNER_API_TOKEN` (+ optional `_SERVER_TYPE` / `_IMAGE` / `_LOCATION` / `_SSH_USER` / `_EXTRA_CLOUD_INIT_WRITE_FILES`)                                                                                                                                                 | Hetzner Cloud console → project → Security → API Tokens (read/write)                                                 | **Needed**                                                                  |
