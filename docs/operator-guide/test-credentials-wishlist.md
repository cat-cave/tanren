...
| **Sentry**        | Sentry internal-integration / personal auth token                    | `tokenRef` in `SentryConfig`; `baseUrl` defaults `https://sentry.io` (`sentryConnector.ts`)                                                                                | Sentry → Settings → Developer/Internal Integrations → auth token (`event:read`/`project:read`) | **Needed**                           |
| **Linear**        | Linear personal API key (or OAuth token)                             | `tokenRef` in `LinearConfig`; `endpoint` defaults `https://api.linear.app/graphql` (`linearConnector.ts`)                                                                  | Linear → Settings → API → Personal API key                                                     | **Needed**                           |
