...
- Every adapter seam gets a **shared conformance test** that _any_ implementation must pass — the in-memory fake, the real one, and a future Rust one. **~21 conformance suites** now live under `services/orchestrator/tests/conformance/`, covering `Allocator`, `SecretStore`, `JobQueue`, `EventStore`, `CostResolver`, the `Repositories` data-access seam, the four VCS/merge seams (`WorkspaceVcsCore`, `CodeHost`, `MergeAuthority`, `VisibilityProjection`), `DagWalker`, `MergeCoordinator`, `CommandSubstrate`, the integration provisioner, and more.
...
- **Baseline + floor:** the first full-scope run measured **39.89%** (926 killed + 64 timed-out of ~2,483 mutants). `thresholds.break` is now pinned at **42** in `stryker.config.mjs` (raised conservatively above the prior 38 floor as the scope tightened to the workflow `planner` / `checker` / `auditor` reasoning modules), so `just mutation` passes today and any regression below the floor fails — a non-breaking ratchet. A precise full re-measure and raising the floor further is follow-up work; the value delivered here is the number plus the regression gate, not fixing every weak test now.
