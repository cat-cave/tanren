...
export default defineConfig({
  ...
  test: {
    reporters: ["default"],
    // Reset the process-wide system-pool memo after every FILE (not every test) so a
    // `beforeAll`-injected BYPASSRLS pool survives all of a file's tests while still
    // never leaking across files. Unit tests inject their in-memory pool as the
    // system pool explicitly (runWithSystemScope now fails loud without one). See
    // test/setup/systemPool.ts + db/src/orgScope.ts.
