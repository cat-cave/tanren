...
- **Toolchain** (oxc / native): **TypeScript 7** (`typescript` package; native `tsc` binary) does typecheck + build; **oxlint** is the fast linter and **`oxlint --type-aware`** (**oxlint-tsgolint**) is the typed pass (config `oxlintrc.typeaware.json`); **oxfmt** is the formatter (`.oxfmtrc.json`, `just format`); **vitest 4** is the test runner; **Turborepo** caches build/typecheck (`.turbo/`). vitest stays the mutation (Stryker, weekly) + e2e runner.
