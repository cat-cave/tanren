...
> **Note (updated):** the two criteria above that require a run to _complete through the dashboard_ (end-to-end operator run; failure-recovery re-plan completing) depended on the run executor, which was deferred and built in Phase 3 as **P3-0001 (now landed)**. Their final validation is the **P3-0009 live demo**, which still needs real credentials to record. The `acceptance-easy`/`-medium` direct-execution harnesses were **removed once P3-0001 landed**; the system is now exercised only through the real dequeue→execute path (`TANREN_RUN_WORKER=1`).
...
**Reconciliation with Phase 2B (resolved):** Phase 2B shipped the operator dashboard _surfaces_, but the orchestrator had **no run executor** (the implicit, un-specced prerequisite Tier 1 named as P3-0001). **P3-0001 has landed**: a background run worker (`TANREN_RUN_WORKER=1`, `services/orchestrator/src/engine/worker/`) claims and executes `plan` jobs, so a dashboard-triggered run runs end-to-end on its own; the direct-execution acceptance scripts (`scripts/acceptance/easy.ts`, `medium.ts`) were **removed** in favor of the real dequeue+execute path. The "completes successfully" / "runs end-to-end" validation claims in P2B-0006/P2B-0008 and the Phase 2 Exit Criteria above are met now that Tier 1 is real. (Full reconciliation in [`docs/roadmap/phase-3-specs.md`](docs/roadmap/phase-3-specs.md).)

Phase 3 closed the v0 workflow above the Phase 2 operator-control baseline, added the remaining providers, brought the hi-fi's deferred surfaces online, hardened deployment, and cleared the audit's remaining medium-priority items. Delivered buckets (merged): workflow completion (review/merge with per-repo configurable integrations · two-tier in-loop gate-checks from a repo-sourced `tanren-ci.yml`, with the checker Answerer reframed to intent/review-only) · thick Forge LLM backend · spec DAG canvas · spec discovery flow · full greenfield + brownfield onboarding · `tanren-config` audit-gate · subscription-window heatmap + DORA · live preview deploys · demo-role LLM wiring · additional workflow insights (stuck, review_stall) · scheduled audits library · issue source ingestion (GitHub/Sentry/Linear/Jira connectors) · external-push governance posture · provider expansion (Claude, opencode-Zai, aider) · notification channel rollout (all 9 channels: ntfy/slack/github-checks/teams/discord/email/twilio/pagerduty/webhook) · acceptance hard tier · allocator expansion (static/sidecar/manual-ssh/Hetzner/DigitalOcean/GCP/AWS-EC2/Kubernetes) · CI/queue hardening · observability · deployment hardening including Authentik OIDC.

**Still genuinely pending/deferred (honest list):** the live demo (P3-0009) and live validation across the expansion seams need real credentials and have not been run; the **agy / pi / reasonix** harnesses await CLI specs (aider is done); the **GitLab / VCS-provider** abstraction is deliberately deferred (GitHub is coupled across PR/merge/CI via Mergify + GitHub Actions); the eight thick-product hi-fi surfaces are built — including Forge in-conversation **write-action** approval (propose → approve → execute under the approving operator's authz; landed in #139). The workspace-bootstrap wiring loose-end is **resolved** (#137): the runner now resolves the per-repo `tanren-ci.yml` `bootstrapCommand` and falls back to `DEFAULT_BOOTSTRAP_COMMAND` only when none is declared. A live run still requires a **fresh/reset dev DB**, since migration `0026` makes `org_id` NOT NULL on the core tables. The next planned-but-unbuilt track is the SaaS **RLS + control/data-plane split** (see [`docs/roadmap/saas-rls-and-plane-split-plan.md`](docs/roadmap/saas-rls-and-plane-split-plan.md), plan-only pending decisions).

PROJECT_BRIEF §3.1 (opencode provider list) was amended at Phase 3 entry to remove the Wafer reference; PROJECT_BRIEF was otherwise treated as fixed during Phase 2 planning. <br/> **SaaS-priming / strictness / longevity expansion (merged)** — tracked in [`docs/roadmap/expansion-and-strictness-plan.md`](docs/roadmap/expansion-and-strictness-plan.md). Beyond the Phase 3 buckets, a broad adapter / strictness / longevity / SaaS-priming expansion shipped on `main`: the **strictness gate** hardened to a 14-step `just fast-check` (format-check, lint, types-lint, architecture, schema/state/event/answerer/contract drift, knip dead-code, spelling, typecheck, test, compose-config) with coverage floors, structural ratchets, and prettier/cspell/commitlint; **tenancy DB-enforced** (mandatory `org_id`) with tenant-namespaced Vault refs (`credential/<slug>/<scope>/<ownerId>/<name>`), a **quota/admission-gate + metering-export seam**, and a **BYOK-vs-managed provider toggle**; pluggable **secret-store backends** (Vault default, GCP Secret Manager, AWS Secrets Manager, 1Password); and identity expanded to `github_oauth` / generic OIDC / **Authentik preset** / `local_dev`.
