...
**Still genuinely pending/deferred (honest list):** the live demo (P3-0009) and live validation across the expansion seams need real credentials and have not been run; the **agy / pi / reasonix** harnesses await CLI specs (aider is done); the **GitLab / VCS-provider** abstraction is deliberately deferred (GitHub is coupled across PR/merge/CI via Mergify + GitHub Actions); the eight thick-product hi-fi surfaces are built but their final interaction model carries one open design item (Forge in-conversation **write-action** approval is still deferred — see [`docs/design/phase-3-hifi-gaps.md`](docs/design/phase-3-hifi-gaps.md)); and one **workspace-bootstrap wiring loose-end** remains — the runner still runs `DEFAULT_BOOTSTRAP_COMMAND` instead of the per-repo `tanren-ci.yml` `bootstrapCommand` resolver (the P3-0004/0006 resolver exists; the run path never feeds it). A live run also requires a **fresh/reset dev DB**, since migration `0026` makes `org_id` NOT NULL on the core tables.
