-- in-1 clean replacement: there are no legacy users or compatibility rows to
-- preserve. The old one-provider authority and under-scoped app-env table are
-- deleted before the lifecycle model is installed; no view, backfill, or dual
-- write survives this migration.
DROP TABLE "org_integrations";
--> statement-breakpoint
DROP TABLE "project_app_env";
--> statement-breakpoint
...
-- Every lifecycle row is a direct tenant root. Dynamic DDL keeps the policy
-- byte-identical across the complete model and makes omission from this closed
-- table list visible in the schema contract test.
DO $$
DECLARE
  lifecycle_table text;
BEGIN
  FOREACH lifecycle_table IN ARRAY ARRAY[
    'behavior_integration_requirements', 'capability_node_dependencies',
    'capability_nodes', 'delivery_runs', 'delivery_stage_attempts',
    'integration_binding_env', 'integration_bindings',
    'integration_reconciliations', 'integration_requirements',
    'integration_resource_snapshots', 'integration_validation_proofs',
    'org_integration_connections', 'org_integration_grants',
    'project_app_env', 'spec_capability_dependencies'
  ] LOOP
    EXECUTE format('ALTER TABLE %I ENABLE ROW LEVEL SECURITY', lifecycle_table);
    EXECUTE format('ALTER TABLE %I FORCE ROW LEVEL SECURITY', lifecycle_table);
    EXECUTE format('DROP POLICY IF EXISTS rls_org_isolation ON %I', lifecycle_table);
    EXECUTE format(
      'CREATE POLICY rls_org_isolation ON %I FOR ALL USING (org_id = current_setting(''app.current_org_id'', true)) WITH CHECK (org_id = current_setting(''app.current_org_id'', true))',
      lifecycle_table
    );
  END LOOP;
END $$;
