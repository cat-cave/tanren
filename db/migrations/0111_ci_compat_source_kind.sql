...
ALTER TABLE "behavior_verification_attempts" ADD CONSTRAINT "behavior_verification_attempts_org_run_id_id_unique" UNIQUE("org_id","run_id","id");--> statement-breakpoint
...
ALTER TABLE "ci_test_results" ADD CONSTRAINT "ci_test_results_behavior_attempt_run_fk" FOREIGN KEY ("org_id","behavior_verification_run_id","behavior_attempt_id") REFERENCES "public"."behavior_verification_attempts"("org_id","run_id","id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
