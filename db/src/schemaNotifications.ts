...
const NOTIFICATION_CHANNEL_KINDS = [
  "ntfy", "slack", "github_checks", "teams", "discord", "email", "twilio", "pagerduty", "webhook"
] as const;
...
export const notificationTargets = pgTable("notification_targets", {
  ...
  check("notification_targets_scope_user_check", sql`(${table.scope} = 'org' AND ${table.userId} IS NULL) OR (${table.scope} = 'user' AND ${table.userId} IS NOT NULL)`),
...
export const notificationRoutes = pgTable("notification_routes", {
  id: text("id").primaryKey(),
  targetId: text("target_id").notNull().references(() => notificationTargets.id),
