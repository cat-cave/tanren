...
export const integrationResourceSnapshots = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      columns: [table.orgId, table.requirementId],
      foreignColumns: [integrationRequirements.orgId, integrationRequirements.id],
      name: "integration_resource_snapshots_requirement_fk",
    }),
    foreignKey({
      columns: [table.orgId, table.bindingId],
      foreignColumns: [integrationBindings.orgId, integrationBindings.id],
      name: "integration_resource_snapshots_binding_fk",
    }),
...
export const deliveryRuns = pgTable(
  "delivery_runs",
  {
    ...
    bindingGenerations: jsonb("binding_generations").notNull(),
    status: text("status").notNull().default("pending"),
    claimOwner: text("claim_owner"),
    claimExpiresAt: timestamp("claim_expires_at", { withTimezone: true }),
    retryAfter: timestamp("retry_after", { withTimezone: true }),
...
export const integrationValidationProofs = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      columns: [table.orgId, table.bindingId],
      foreignColumns: [integrationBindings.orgId, integrationBindings.id],
      name: "integration_validation_proofs_binding_fk",
    }),
    foreignKey({
      columns: [table.orgId, table.deliveryRunId],
      foreignColumns: [deliveryRuns.orgId, deliveryRuns.id],
